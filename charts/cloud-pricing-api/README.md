...
2. You can then configure the Infracost CLI to use the self-hosted Cloud Pricing API using the following two environment variables. Your Infracost CLI users will use the API key to authenticate when calling your self-hosted Cloud Pricing API
    ```sh
    export INFRACOST_PRICING_API_ENDPOINT=http://localhost:4000

    export INFRACOST_API_KEY=$(kubectl get secret --namespace $NAMESPACE cloud-pricing-api --template="{{ index .data \"self-hosted-infracost-api-key\" }}" | base64 -D)
